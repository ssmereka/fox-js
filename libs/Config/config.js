...
var createConfigObject = function(_userConfig, _systemConfig) {
  ...
  // Load the merge library if it hasn't already been loaded.
  merge = loadMergeLibrary();
...
/**
 * Load the merge utility if it has not already been loaded.
 * Note this will not pass in the fox instance so you must 
 * update the library once you have a valid fox instance.
 */
var loadMergeLibrary = function() {
  // Return the already loaded merge object.
  if(merge) {
    return merge;
  }

  // Find and load merge.
  Merge = require(path.resolve(__dirname, "../Utility/merge.js"));
  merge = new Merge();
